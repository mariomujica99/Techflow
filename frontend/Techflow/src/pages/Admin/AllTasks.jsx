...
const AllTasks = () => {
  ...
  useEffect(() => {
    const hasSeenWarning = sessionStorage.getItem('disconnectedWarning_AllTasks');
    
    if (filterStatus === "Disconnected" && !hasSeenWarning && disconnectedCount >= 15) {
      setShowDisconnectedWarning(true);
      sessionStorage.setItem('disconnectedWarning_AllTasks', 'true');
    }
  }, [filterStatus, disconnectedCount]);

  useEffect(() => {
    const hasSeenWarning = sessionStorage.getItem('completedWarning_AllTasks');
    
    if (filterStatus === "Completed" && !hasSeenWarning && completedCount >= 25) {
      setShowCompletedWarning(true);
      sessionStorage.setItem('completedWarning_AllTasks', 'true');
    }
    ...
    <DashboardLayout activeMenu="All Tasks">
      <div className="mt-2.5 mb-5">
        <div className="flex flex-col lg:flex-row lg:items-center justify-between">
          <h2 className="text-xl md:text-xl text-gray-500 font-bold flex-shrink-0 whitespace-nowrap mr-10">All Tasks</h2>
